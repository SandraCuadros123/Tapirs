...
familyunits <- read.csv("C:/Users/sandracd/OneDrive - UBC (1)/GitHub/Tapirs/family units hierarchy2.csv")

#load UDS object
UDS <- readRDS("C:/Users/sandracd/OneDrive - UBC (1)/GitHub/Tapirs/Tapirs_UDS.rds") %>%
  map(\(ud) { 
...
#Another way to read it using Ade package instead in case above doesnt work
UDS <- readRDS("C:/Users/sandracd/OneDrive - UBC (1)/GitHub/Tapirs/Tapirs_UDS.rds") %>%
  map(\(ud) { 
    adehabitatHR::getverticeshr(ud) %>%  # UD to spatial polygon
      st_as_sf() %>%                     # convert to simple feature
      st_transform(crs = '+proj=longlat') %>%  # project to lat/long
      return()
  }) %>%
  bind_rows() %>%
  mutate(
    est = grepl(pattern = 'est', x = name),
    tapir = map_chr(name, ~ substr(.x, 1, gregexpr(" ", .x)[[1]][1] - 1))
  ) %>%
  filter(est) %>%
  left_join(y = familyunits, by = 'tapir')
...
#Load data
t_relatedness_detailed <- read.csv("C:/Users/sandracd/OneDrive - UBC/GitHub/Tapirs/t_relatedness_detailed.csv")
...
#Make text smaller
FIG_family <- FIG_family + theme(legend.text = element_text(size = 12))
FIG <- FIG + theme(legend.text = element_text(size = 6))

#plot both  figs together
FIG2 <- grid.arrange(FIG, FIG_family, ncol = 2)
FIG2 <- ggarrange(
  ggarrange(FIG, FIG_family, 
            align = "v", 
            ncol=2, 
            heights = c(6,7), 
            widths = c(6,7), 
...
#save plots  
ggsave(FIG2, file = "FIG 2.png", path = 'C:/Users/sandracd/OneDrive - UBC/GitHub/Tapirs/Manuscript', width = 6.86, height = 8, units = "in", dpi = 600)
ggsave(FIG2, file = "FIG 2 trial 2.png", path = 'C:/Users/sandracd/OneDrive - UBC/GitHub/Tapirs/Manuscript', width = 10, height = 8, units = "in", dpi = 600)
